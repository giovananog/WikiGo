...
public class EsClient {
    ...
    public SearchResponse<ObjectNode> executeSearchQuery(Query finalMatchQuery, Integer page) {
        try {
    ...
    public SearchResponse searchWithoutFilters(String query, Integer page) {
        ...
        if (queryWithoutMarks.startsWith("\"") && queryWithoutMarks.endsWith("\"")) {
            ...
            return executeSearchQuery(matchPhraseQuery, page);
        } else {
            Query matchQuery = MatchQuery.of(q -> q.field("content").query(query))._toQuery();
            return executeSearchQuery(matchQuery, page);
        }
    }

    //  search with operator 'and'
    public SearchResponse searchWithOperatorAnd(String query, Integer page, String... filters) {
        Query matchQuery = MatchPhraseQuery.of(q -> q.field("content").query(query))._toQuery();
        return executeSearchQuery(matchQuery, page);
    }

    //  search with boolQuery 'mustNot'
    public SearchResponse searchWithMustNot(String query, Integer page, String... filter) {
    ...
    public SearchResponse<ObjectNode> searchWithDtCreation(String query, Integer page, String... filters) {
    ...
    public SearchResponse<ObjectNode> searchWithReadingTime(String query, Integer page, String... filters) {
        logger.info("Executing search with dt_creation filter: query={}, filters={}", query, filters);
        String comparison = filters[0];
        String date = filters[1];
        Query matchQuery;
    ...
    public SearchResponse searchWithFuzziness(String query, Integer page, String... filter) {
